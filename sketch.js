...
function createSilhouette() {
  ...
  // 轉換為灰度圖
  silhouetteCanvas.filter(GRAY);
  
  // 應用適度模糊效果
  silhouetteCanvas.filter(BLUR, 3);
  ...
  for (let i = 0; i < silhouetteCanvas.width; i++) {
    for (let j = 0; j < silhouetteCanvas.height; j++) {
      ...
      if (alpha > 128 && brightness < 200) {
        // 非透明且較暗的部分，轉為黑色
        silhouetteCanvas.pixels[pixelIndex] = 20;
        silhouetteCanvas.pixels[pixelIndex + 1] = 20;
        silhouetteCanvas.pixels[pixelIndex + 2] = 20;
        silhouetteCanvas.pixels[pixelIndex + 3] = 255;
      } else {
        // 背景保持白色
        silhouetteCanvas.pixels[pixelIndex] = 255;
        silhouetteCanvas.pixels[pixelIndex + 1] = 255;
        silhouetteCanvas.pixels[pixelIndex + 2] = 255;
        silhouetteCanvas.pixels[pixelIndex + 3] = 255;
      }
    }
  }
